      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'url'">
          <TableImg
            ...
            simpleShow
          />
          <Tooltip v-else :title="record.url" :placement="'topLeft'">{{ record.url }}</Tooltip>
        </template>
...
<script setup lang="ts">
  ...
  const { txt, doc, docx, xls, xlsx, ppt, pptx, pdf } = MimeType;
  ...
  onMounted(async () => {
    const ret = await getConfigByKey('sys.oss.previewListResource');
    if (ret.code === 200) {
      preview.value = ret.data === 'true' ? true : false;
    }
  });
