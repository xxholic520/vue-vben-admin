<template>
  <PageWrapper dense>
          ...
          <template #tableTitle>
            <Space>
              ...
              <a-button
                ...
                danger
                @click="multipleRemove(removeUser)"
          ...
          <template #toolbar>
            <Space>
              ...
              <a-button
                ...
                @click="
                  downloadExcel(
                    () => exportUser(getForm().getFieldsValue() as UserQuery),
                    '用户数据.xlsx',
                  )
                "
                ghost
    ...
    <UserModal @register="registerModal" @reload="reload" />
...
<script setup lang="ts">
  ...
  import { getUserDeptTree, getUserList, exportUser, removeUser } from '@/api/system/user';
  ...
  onMounted(async () => {
    data.tree = await getUserDeptTree();
  });

  const [registerTable, { reload, getSelectRowKeys, multipleRemove, getForm }] = useTable({
    rowSelection: {
      type: 'checkbox',
