...
<script setup lang="ts">
  ...
  const [registerTable, { reload, getSelectRowKeys, clearSelectedRowKeys, getForm }] = useTable({
    rowSelection: {
      type: 'checkbox',
    },
    api: generatedList,
  ...
  const reloadWithCallback = (callback: Fn) => {
    return reload().then(callback);
  };
  ...
  async function handleMultipleDelete() {
    ...
    createConfirm({
      ...
      iconType: 'warning',
      onOk: async () => {
        await genRemove(selectedRowKeys);
        clearSelectedRowKeys();
        await reload();
      },
    });
