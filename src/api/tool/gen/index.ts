...
// 修改代码生成业务
export function genInfo(tableId: string | number) {
...
// 查询数据表字段列表
export function columnList(tableId: string | number) {
...
// 删除代码生成
export function genRemove(tableIds: Array<string | number>, successMsg = true) {
...
// 预览代码
export function preview(tableId: string | number) {
  return defHttp.get<any[]>({ url: Api.preview + '/' + tableId });
}

// 生成代码（下载方式）
export function genDownload(tableId: string | number) {
  return defHttp.get({ url: Api.download + '/' + tableId });
}

// 生成代码（自定义路径）
export function genDownloadWithPath(tableId: string | number) {
  return defHttp.get({ url: Api.download + '/' + tableId });
}

// 同步数据库
export function syncDb(tableId: string | number, successMsg = true) {
  return defHttp.get(
...
// 批量生成代码
export function batchGenCode(tableIdStr: string | number | Array<string | number>) {
